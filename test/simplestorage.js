...
contract("SwapContract", (accounts) => {
  ...
  it("should read & update token conversion rate", async () => {
    ...
    let conversionRate = await swapContract.getTokenConversionRate(
      tokenCZH.address
    );
    assert.equal(conversionRate.toString(), initialTokenConversionRate);
    await swapContract.updateTokenConversionRate(
      tokenCZH.address,
      lastTokenConversionRate
    );
    conversionRate = await swapContract.getTokenConversionRate(
      tokenCZH.address
    );
    assert.equal(conversionRate.toString(), lastTokenConversionRate);
    await swapContract.updateTokenConversionRate(
      tokenCZH.address,
      initialTokenConversionRate
    );
  });
  ...
  it("should transferErc20 to account from swapContract and improve account token amount and reduce token amount on swap", async () => {
    ...
    await swapContract.transferERC20(
      tokenCZH.address,
      accounts[0],
      tokenTransferAmount
    );
    const accountBalanceCZH = await tokenCZH.balanceOf(accounts[0]);
    const swapContractBalanceCZH = await tokenCZH.balanceOf(
      swapContract.address
    );

    assert.equal(accountBalanceCZH.toString(), tokenTransferAmount);
    assert.equal(
      web3.utils.fromWei(swapContractBalanceCZH, "ether"),
      web3.utils.fromWei(initialSupplyAmount, "ether") -
        web3.utils.fromWei(tokenTransferAmount, "ether")
    );
  ...
  it("should withdraw", async () => {
    let initialEthBalanceOnSwap = web3.utils.fromWei(
      await web3.eth.getBalance(swapContract.address),
      "ether"
    );
    await web3.eth.sendTransaction({
      from: accounts[0],
  ...
  it("should buyTokens - 100 CZH for 1 ETH of Account", async () => {
    ...
    let czhConversionRateForOneEth = await swapContract.getTokenConversionRate(
      tokenCZH.address
    );
    let one_eth = web3.utils.toWei("1", "ether");

    let accountBalanceCZH = web3.utils.fromWei(
      await tokenCZH.balanceOf(accounts[0]),
      "ether"
    );

    await swapContract.buyTokens(tokenCZH.address, {
      from: accounts[0],
      value: one_eth,
    });
    accountBalanceCZH = web3.utils.fromWei(
      await tokenCZH.balanceOf(accounts[0]),
      "ether"
    );
  ...
  it("should sellTokens", async () => {
    ...
    await swapContract.sellTokens(
      tokenCZH.address,
      accountBalanceCZH.toString()
    );
  ...
  it("should swapTokens", async () => {
    ...
    let lfjConversionRate = await swapContract.getTokenConversionRate(
      tokenLFJ.address
    );
    ...
    // swapping the token
    await tokenCZH.approve(
      swapContract.address,
      await tokenCZH.balanceOf(accounts[0])
    );
    await swapContract.swapTokens(
      tokenLFJ.address,
      tokenCZH.address,
      await tokenCZH.balanceOf(accounts[0])
    );
    const newCzhBalance = web3.utils.fromWei(
      await tokenCZH.balanceOf(accounts[0]),
      "ether"
    );
    const newLfjBalance = web3.utils.fromWei(
      await tokenLFJ.balanceOf(accounts[0]),
      "ether"
    );
